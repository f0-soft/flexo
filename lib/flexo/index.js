	...
	for ( i = 0; i < request.query.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			if ( dict.mutable.indexOf( keys[j] ) === -1 ) {
				return next( callback, myErr( 'insert, схема `' + request.name + '`, documents.' + i + ' - указано значение неизменяемого поля `' + keys[j] + '`. Документ: ' + JSON.stringify( doc ) ) );
			}

			// check types, force arrays, validate
			valid = VALIDATOR.toBase( doc[keys[j]], dict.types[keys[j]].type, dict.types[keys[j]].subtype );
			if ( valid[0] ) { return next( callback, myErr( 'insert, схема `' + request.name + '`, documents.' + i + ' - ошибка данных в поле `' + keys[j] + '` - ' + valid[0] ) ); }

			// clone property;
			docs[i][keys[j]] = valid[1];
		}
	}
...
// возвращает функцию, сериализующую объект ошибки перед вызовом коллбека
// коллбек - последний аргумент вызова, ошибка - первый аргумент коллбека
...
//TODO: разобраться, где и как dnode сериализует данные ответа, почему не вызывает метод toJSON
//Решение этой проблемы позволит в один раз изменить прототип Error вместо того, чтобы оборачивать функции и подменять коллбеки
/*
...
init = function( options, callback ) {
	...
	for ( var j = 0; j < keys.length; j += 1 ) {
		upContainer[keys[j]] = wrapCallbackError( container[ keys[j] ] );
	}
