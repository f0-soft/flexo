...
function processResponse( data, fields, types ) {
	...
	for ( i = 0; i < data.length; i += 1 ) {
		...
		for ( j = 0; j < fields.length; j += 1 ) {
			res = flexoValidator.toUser( data[i][fields[j]], types[fields[j]].type, types[fields[j]].subtype || null );
...
container.aggregate = function( request, callback ) {
	...
	return STORAGE.aggregate( { collname: request.name, pipeline: request.pipeline }, function( err, data ) {
		...
		for ( i = 0; i < dict.length; i += 1 ) {
			key = dict[i][0];
			field = dict[i][1];
			res = flexoValidator.toUser( data[0][ key ], types[ field ].type, types[ field ].subtype || null );
...
container.insert = function( request, callback ) {
	...
	for ( i = 0; i < request.query.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			valid = flexoValidator.toBase( doc[key], dict.types[key].type, dict.types[key].subtype );
...
container.modify = function( request, callback ) {
	...
	for ( i = 0; i < request.query.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			// check types, force arrays, validate
			valid = flexoValidator.toBase( request.query[i].properties[keys[j]], dict.types[keys[j]].type, dict.types[keys[j]].subtype );
...
function newDocument( fields, types ) {
	var i, out = {};
	for ( i = 0; i < fields.length; i += 1 ) {
		out[ fields[i] ] = flexoValidator.defaults[ types[ fields[i] ].type ]();
	}
	return out;
}
