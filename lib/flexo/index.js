	...
	return async.auto( tasks, function( err, results ) {
		...
		return runHooks( hooks, { schemes: SCHEMES, db: container, request: request }, function( err ) {
			...
			return STORAGE.insert( request.name, docs, function( err, result ) {
				...
				return runHooks( hooks, { schemes: SCHEMES, db: container, result: result }, function( err ) {
					var documents;
...
function runHooks( hooks, self, callback ) {
	...
	var i, j, k, _self, tOut, tasks = [];
	var keys, subkeys;

	if ( errType ) { return next( callback, errType ); }

	keys = Object.keys( self );
	subkeys = self.request ? Object.keys( self.request ) : [];

	for ( i = 0; i < hooks.length; i += 1 ) {
		// клонирование контейнера
		if ( self.result ) {
			for ( j = 0; j < self.result.length; j += 1 ) {
				_self = {};
				for ( k = 0; k < keys.length; k += 1 ) { _self[keys[k]] = self[keys[k]]; }

				_self.result = self.result[j];

				// постановка в очередь
				tasks.push( hooks[i].bind( _self ) );
			}
		} else {
			for ( j = 0; j < self.request.query.length; j += 1 ) {
				_self = {};
				for ( k = 0; k < keys.length; k += 1 ) { _self[keys[k]] = self[keys[k]]; }

				_self.request = {};
				for ( k = 0; k < subkeys.length; k += 1 ) {
					_self.request[subkeys[k]] = self.request[subkeys[k]];
				}
				_self.request.query = self.request.query[j];
