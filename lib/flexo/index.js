...
container.find = function( request, callback ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		if ( typeof request.query[keys[i]] !== 'object' && !flexoValidator.checkType[types[keys[i]].type]( request.query[keys[i]] ) ) {
...
container.aggregate = function( request, callback ) {
	...
	return STORAGE.aggregate( [ request.name, request.pipeline ], callback );
};
...
container.modify = function( request, callback ) {
	...
	for ( i = 0; i < request.query.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			// check types, validate
			valid = flexoValidator.validate( request.query[i].properties[keys[j]], scheme.root[keys[j]] );
			if ( valid.length !== 0 ) {
				return next( callback, valid );
			}
	...
	return async.parallel( tasks, function( err, data ) {
		...
		return runHooks( hooks, {db: container, schemes: SCHEMES, request: request}, function( err ) {
			if ( err ) { return callback( err ); }
