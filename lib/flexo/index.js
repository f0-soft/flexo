...
container.aggregate = function( name, match, group, callback ) {
	STORAGE.aggregate( [name, selectorObjectToArray( match ), group], callback );
};
...
container.insert = function( name, fields, documents, options, callback ) {
	...
	for ( i = 0; i < documents.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			if ( valid.length !== 0 ) {
				return process.nextTick( callback.bind( null, myErr( 'ошибка данных - ' + JSON.stringify( valid ) ) ) );
			}
...
function selectorObjectToArray( selector ) {
	...
	while ( queue.length > 0 ) {
		...
		for ( i = 0; i < keys.length; i += 1 ) {
			...
			if ( typeof node.elem[keys[i]] === 'object' ) {
				node.out.push( [] );
