...
container.insert = function( name, fields, documents, options, callback ) {
	...
	for ( i = 0; i < documents.length; i += 1 ) {
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			...
			if ( valid.length !== 0 ) {
				return process.nextTick( callback.bind( null, [key, valid] ) );
			}
...
container.modify = function( name, query, options, callback ) {
	...
	return async.parallel( tasks, function( err, data ) {
		...
		return STORAGE.modify( name, query, function( err, data ) {
			var i, j, res = [];
			if ( err ) { return callback( err ); }

			for ( i = 0; i < data.length; i += 1 ) {
				for ( j = 0; j < query.length; j += 1 ) {
					if ( data[i][DOC_ID] === query[j].selector[DOC_ID] ) {
						res.push( {} );
						res[res.length - 1][DOC_ID] = data[i][DOC_ID];
						res[res.length - 1][DOC_UPDATE] = query[j].properties[DOC_UPDATE];
						break;
					}
				}
			}
