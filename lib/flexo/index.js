...
container.find = function( name, fields, query, options, callback ) {
	...
	if ( errType ) { return process.nextTick( callback.bind( null, errType ) ); }
...
container.insert = function( name, fields, documents, options, callback ) {
	...
	return async.auto( tasks, function( err, results ) {
		...
		if ( scheme.before && scheme.before.insert ) {
			for ( i = 0; i < scheme.before.insert.length; i += 1 ) {
				tasks.push( scheme.before.insert[i].bind( {schemes: SCHEMES, db: container}, name, docs ) );
			}
		}
		...
		return async.parallel( tasks, function( err ) {
			...
			return STORAGE.insert( name, docs, function( err, result ) {
				...
				if ( scheme.after && scheme.after.insert ) {
					for ( i = 0; i < scheme.after.insert.length; i += 1 ) {
						tasks.push( scheme.after.insert[i].bind( {schemes: SCHEMES, db: container}, name, docs ) );
					}
				}
...
container.modify = function( name, query, options, callback ) {
	...
	return async.parallel( tasks, function( err, data ) {
		...
		if ( scheme.before && scheme.before.modify ) {
			for ( i = 0; i < scheme.before.modify.length; i += 1 ) {
				tasks.push( scheme.before.modify[i].bind( {db: container, schemes: SCHEMES}, name, query ) );
			}
		}
		...
		return async.parallel( tasks, function( err ) {
			...
			return STORAGE.modify( name, query, function( err, result ) {
				var i, tasks = [];
			
					if ( err ) { return callback( err ); }

				if ( scheme.after && scheme.after.modify ) {
					for ( i = 0; i < scheme.after.modify.length; i += 1 ) {
						tasks.push( scheme.after.modify[i].bind( {db: container, schemes: SCHEMES}, name, query ) );
					}
				}
