...
ARRAYS['objarray'] = true;
toBase.objarray = function( elem, subtype ) {
	var keys, subresult, result, obj;

	result = toBase.array( elem );
	...
	for ( var i = 0; i < result[1].length; i += 1 ) {
		...
		for ( var j = 0; j < keys.length; j += 1 ) {
			subresult = processToBase( result[1][i][ keys[j] ], subtype[ keys[j] ] );
...
toUser.objarray = function( elem, subtype ) {
	var keys, subresult, result, obj;

	result = toUser.array( elem );
	if ( result[0] ) { return result; }
	...
	for ( var i = 0; i < result[1].length; i += 1 ) {
		obj = {};
		for ( var j = 0; j < keys.length; j += 1 ) {
			subresult = processToUser( result[1][i][ keys[j] ], subtype[ keys[j] ] );
