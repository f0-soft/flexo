...
toQuery.str = function( query ) {
	...
	var cmd = reopcode.decode( str.charCodeAt( 0 ) );
	str = processToBase( str.substr( 1 ), 'str' );
...
toQuery.words = function( query ) {
	var wrds = query.toString();
	var cmd = reopcode.decode( wrds.charCodeAt( 0 ) );
	wrds = processToBase( wrds.substr( 1 ), 'words' );
