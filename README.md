...
## container.groupCount( request, callback )
...
* `request` - объект
    * `parent` - объект, содержит информацию по родительской коллекции
        ...
        * `field_link` - строка, поле связи, у родителя `_id`
        * `selector` - объект, селектор rabbit
    * `child` - объект
        * `coll` - строка, название коллекции
        * `field_sum` - строка, поле сравнения значений
        * `field_link` - строка, поле связи, у ребенка что-то вроде `parent_id`
        * `selector` - объект, селектор rabbit
    * `groups` - массив
...
request.groups = [
    { $gt: 100 }, // оплата более, чем на 100%
    { $eq: 100 }, // оплата на 100%
    { $lt: 100, $gte: 75}, // оплата на 75-100%
    { $lt: 75, $gt: 0 }, // оплата на 0-75%
...
```
var result = [
    { count: 1, parentSum: 10, childSum: 15 }, // оплата более, чем на 100%
    { count: 2, parentSum: 5, childSum: 5 }, // оплата на 100%
    { count: 0, parentSum: 0, childSum: 0 }, // оплата на 75-100%
    { count: 0, parentSum: 0, childSum: 0 }, // оплата на 0-75%
    { count: 0, parentSum: 0, childSum: 0 } // счета без оплаты
];
```
