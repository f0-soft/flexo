...
exports.before: { // может содержать только `insert`, `modify`
    insert: [ // функции в массиве запускаются через async.parallel
        function( name, docs, callback ) { return callback( null, true ); }
    ],
    modify: [
        function( name, query, callback ) { return callback( null, true ); }
    ]
};
