...
module.exports = {
    ...
    root: {
        ...
        p_id: { type: 'id', title:'Ссылка на продукцию', link: 'productsGroup', from: 'products', includeWeight: ['products']},
...
function insertBills (cb) {
    ...
    this.db.find( queryFind, function( err, data ) {
        ...
        if(!this.query.count) this.query.count = 1;
        else this.query.count = parseInt(this.query.count);
...
function modifyBills (cb) {
    ...
    this.db.find( queryFind, function( err, data ) {
        ...
        var price = parseInt(data.result[0].price*100);
        if(!this.query.properties.count) this.query.properties.count = 1;
        else this.query.properties.count = parseInt(this.query.properties.count);
